...
export default class CartController {
    ...
    async getCartDetail(request: Request, response: Response, next: NextFunction) {
        const cart = await this._cartService.getCartDetail(parseInt(request.params.id));
    ...
    async deleteCartDetail(request: Request, response: Response, next: NextFunction) {
        const cart = await this._cartService.deleteCartDetail(parseInt(request.params.id));
        if (!cart) return response.status(HttpStatus.NOT_FOUND).send(HttpStatus[`${HttpStatus.NOT_FOUND}_MESSAGE`]);

        await this._cartService.deleteCartDetail(parseInt(request.params.id));
    ...
    async updateCart(request: Request, response: Response, next: NextFunction) {
        ...
        const updatedCart: CartResponseDto = await this._cartService.updateCart(parseInt(request.params.id),input);
